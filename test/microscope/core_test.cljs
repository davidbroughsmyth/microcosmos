...
(defn fake-queue [{:keys [mocked initial-state-val] :as f}]
  (when mocked
...
(deftest testing-code
  (testing "using mocked components only"
    (c-macros/mocked
    ;  (io/send! @queue {:payload "Some msg"})
     (is (re-find #"ERROR: Message\n\nCID:"
          (with-out-str
            (a-function)
            (io/send! @queue {:payload "Some msg"})))))))
;
...
; (comment
; (let [function components/params-for-generators]
;   function)
;
; (println "\n\n\n*********************" (gensym))
; (prn
